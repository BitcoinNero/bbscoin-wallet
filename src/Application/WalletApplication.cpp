...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("bitcoinnerowallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("bitcoinnerowallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Bitcoin Nero wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Bitcoin Nero wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have bytecoind with non-default RPC port, you should terminate it and relaunch BitcoinNeroWallet\n"
      "or\n"
      "Set the Local deamon required port in BitcoinNeroWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
