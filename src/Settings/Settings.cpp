...
const char DEFAULT_WALLET_FILE_NAME[] = "BitcoinNeroWallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "nero.getpool.org:58261";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("BitcoinNeroWallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("BitcoinNeroWallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("BitcoinNeroWallet.desktop");
  ...
  res = autorunSettings.contains("BitcoinNeroWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("BitcoinNeroWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.bitcoinnero.bitcoinnero scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Bitcoin Nero Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("BitcoinNeroWallet", appPath);
    } else {
      autorunSettings.remove("BitcoinNeroWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinnero", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:bitcoinnero");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinnero\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoinnero\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("BitcoinNeroWallet.cfg"));
